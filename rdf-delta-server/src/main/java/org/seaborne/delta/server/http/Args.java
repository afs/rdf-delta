...
public class Args {
    ...
    public static Args args(HttpServletRequest request) {
        String ref = request.getParameter(C.paramRef);
        if ( ref != null )
            return registration.get(ref);
        String zone = request.getParameter(C.paramZone);
        String dataset = request.getParameter(C.paramDataset);
        String patchId = request.getParameter(C.paramPatch);
        String version = request.getParameter(C.paramVersion);
